...
برای Multi-Company، قاعده قطعی:

> در هر Company حداکثر یک Position فعال می‌تواند برای Personnel به‌عنوان Primary انتخاب شود.

به دلیل اینکه Company از `Position.CompanyId` به دست می‌آید، در V1 عمداً `CompanyId` را داخل `PersonnelPosition` duplicate نمی‌کنیم. بنابراین این invariant باید توسط یک Command واحد و تراکنشی به نام مفهومی `SetPrimaryPosition` enforce شود؛ یک check ساده قبل از `SaveChanges` کافی و قابل قبول نیست، چون دو درخواست همزمان می‌توانند هر دو check را پاس کنند.

### 14.1.1 حفاظت تراکنشی در برابر Race Condition

`SetPrimaryPosition(PersonnelId, PositionId)` باید در یک Transaction اجرا شود و عملیات همزمان برای همان Personnel را serialize کند. ساده‌ترین مدل قابل دفاع:

```text
1. Begin transaction.
2. Lock/serialize the target Personnel aggregate for update
   (or use an equivalent SERIALIZABLE / concurrency-token strategy).
3. Load target Position and determine CompanyId.
4. Validate Position is ACTIVE and assignment belongs to the Personnel.
5. Load active PersonnelPosition rows of that Personnel whose Position.CompanyId == target CompanyId.
6. Set IsPrimary=false for any current primary in that Company.
7. Set the target assignment IsPrimary=true.
8. Write Audit event in the same transaction.
9. Commit.
```

قواعد غیرقابل مذاکره:

- تمام مسیرهای تغییر Primary باید فقط از همین Command عبور کنند؛
- `IsPrimary` نباید با update مستقیم از UI/Repository عمومی تغییر کند؛
- concurrent commands برای یک Personnel نباید بتوانند دو Primary فعال در یک Company تولید کنند؛
- Primaryهای Companyهای مختلف مستقل‌اند؛ بنابراین یک Personnel می‌تواند در Company A و Company B هرکدام یک Primary داشته باشد؛
- در failure یا concurrency conflict، عملیات باید fail/retry کنترل‌شده داشته باشد و هرگز state دو-Primary commit نشود؛
- تغییر Primary و Audit آن باید atomic باشند.

اگر Provider از pessimistic row locking پشتیبانی کند، lock روی رکورد `Personnel` ساده‌ترین serialization key است. اگر Provider/architecture optimistic concurrency را ترجیح دهد، `RowVersion/ConcurrencyToken` روی Personnel با retry محدود قابل قبول است. انتخاب مکانیزم فنی می‌تواند provider-specific باشد، اما invariant و atomicity نباید تغییر کنند.

اگر در آینده نیاز شدید به enforcement مستقیم schema ایجاد شد، می‌توان read/denormalized key مناسب برای `(PersonnelId, CompanyId)` اضافه کرد و unique constraint ساخت؛ در V1 برای حفظ سادگی، Company روی `PersonnelPosition` تکرار نمی‌شود.
...
# 27. Action implication، prerequisite و DENY

## 27.0 تصمیم قطعی معماری — Cross-Level DENY + Implication

این سند **گزینه ۱** را به‌صورت قطعی انتخاب می‌کند:

> اگر یک Action قوی‌تر برای مؤثر بودن به Permission/Action دیگری نیاز داشته باشد، DENY آن prerequisite در هر ancestor همان Role branch و همان scope، Action قوی‌تر descendant را نیز غیرمؤثر می‌کند؛ حتی اگر ALLOW Action قوی‌تر مستقیماً و مستقل روی descendant ثبت شده باشد.

بنابراین `DENY Products.Read` روی Role C و `ALLOW Products.Edit` مستقیم روی descendant آن Role D در همان scope الزاماً نتیجه `D.CanRead=false` و `D.CanEdit=false` دارد. این رفتار بخشی از Specification است، نه صرفاً Test expectation.

این تصمیم فقط داخل همان Role branch/scope اعمال می‌شود و Source/branch مستقل دیگر User را globally revoke نمی‌کند.

`Edit => Read` فقط Policy runtime است و هیچ Permission اضافی در دیتابیس ایجاد نمی‌کند.

قاعده امنیتی قطعی:

```text
DENY Read
=> actions requiring Read are ineffective in the same role branch and scope
```

این Rule فقط برای DENY و ALLOW روی یک Role نیست. **DENY روی هر ancestor در مسیر همان branch نیز باید prerequisite را برای descendant مسدود کند.**

مثال هم‌سطح:

```text
Role D:
DENY  Products.Read @ Workshop-X
ALLOW Products.Edit @ Workshop-X
```
...
مثال اجباری ancestor/descendant:
...
دلیل: `Products.Edit` برای مؤثر بودن به `Products.Read` وابسته است. DENY روی `Read` در ancestor یک DENY Boundary برای همان branch/scope می‌سازد؛ بنابراین ALLOW مستقیم `Edit` روی descendant حق ندارد prerequisite مسدودشده را دور بزند.

این رفتار باید برای تمام prerequisiteهای Action عمومی باشد، نه فقط Read/Edit. قاعده عمومی:

```text
DENY(Q) on any role in path [Descendant ... Ancestor]
AND
Q is a direct or transitive prerequisite of P
AND
ALLOW(P) originates in that descendant branch

=> P is ineffective in that branch/scope
```

ترتیب ارزیابی هر candidate permission در یک Role branch باید مفهوماً چنین باشد:

```text
1. Resolve the candidate ALLOW and its origin role.
2. Check DENY for the requested permission on the complete path
   from origin role up to and including the consuming/evaluated role.
3. Check DENY for the candidate permission on the same complete path.
4. Resolve all direct/transitive prerequisites of the candidate.
5. For every prerequisite, check DENY on the same complete path.
6. Only if all DENY gates pass may the candidate ALLOW become effective.
7. Otherwise fail closed.
```

مسیر بررسی **هر دو endpoint و تمام Roleهای بین آنها را شامل می‌شود**. پس DENY روی خود descendant، روی ancestor نزدیک، یا روی هر Role میانی یکسان enforce می‌شود.

RoleGroup ALLOW نیز اگر از همان Role branch مؤثر می‌شود، تابع همین prerequisite/DENY gates است و نمی‌تواند این Boundary را دور بزند.

Implication هرگز نباید به‌صورت fallback عمومی پس از ارزیابی DENY اجرا شود.
...
- [ ] دو `SetPrimaryPosition` همزمان برای دو Position در یک Company هرگز دو Primary فعال تولید نمی‌کنند؛ پس از اتمام فقط یکی Primary است و conflict/retry قابل مشاهده و کنترل‌شده است.
- [ ] `SetPrimaryPosition` همزمان برای Positionهای دو Company مختلف می‌تواند در پایان یک Primary مستقل برای هر Company داشته باشد.
- [ ] تغییر Primary و Audit مربوط به آن در یک Transaction commit یا rollback می‌شوند.
...
- [ ] DENY Read + ALLOW Edit روی همان Role/branch => Read=false, Edit=false.
- [ ] **Ancestor prerequisite DENY blocks descendant stronger ALLOW:** اگر Role C دارای `DENY Products.Read @ Workshop-X` باشد و descendant آن Role D مستقیماً `ALLOW Products.Edit @ Workshop-X` داشته باشد، نتیجه برای D الزاماً `Read=false` و `Edit=false` است.
- [ ] DENY یک prerequisite روی هر Role میانی مسیر، candidate stronger action را در descendant همان branch/scope غیرمؤثر می‌کند.
- [ ] prerequisiteهای transitive نیز تحت همان ancestor DENY path بررسی می‌شوند.
